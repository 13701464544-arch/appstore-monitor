...
def get_summary(tag):
    return f"【{tag}】更新审核、合规、隐私政策，调整推荐算法与流量分配，加强SDK、广告、权限行为管控。"

def get_impact(tag):
    if tag == "App Store":
        return "影响iOS应用上架审核、合规成本、流量策略，全球开发者需调整产品设计、运营与推广方案。"
    elif tag == "Google Play":
        return "影响Google Play全球分发、合规要求、算法推荐，直接决定出海应用上架、推广与运营效果。"
    elif tag == "OPPO软件商店":
        return "影响OPPO应用审核、流量分配、推广资源，开发者需适配新规以获取更多曝光与转化机会。"
    elif tag == "VIVO应用商店":
        return "影响VIVO应用上架门槛、合规检测、推荐机制，重塑国内应用分发、流量与竞争格局。"
    elif tag == "小米应用商店":
        return "影响小米应用算法权重、审核规则、流量优先级，推动行业走向合规化、高质量化发展。"
    return "影响应用商店审核、合规、流量策略，开发者需全面调整上架、运营与适配方案。"
...
def get_news():
    return [
        ...
        {"tag":"小米应用商店","title":"小米应用商店严控SDK合规，打击违规数据采集"},
